...
public class MainBaseAI : Building
{
    ...
    public void SubtarctResources(AmountOfResources amount)                 // ѕеренести в Store?
    {
    ...
    public AmountOfResources GetPriceOf(SelectableObject obj)                       // ¬ынести в отдельную сущность
    {
    ...
    private void StartPriceList()                                                       // ѕрайс лист вынести в отдельную сущность
    {
    ...
    private void FindFreeResources()                                // переделать дл€ корректной работы с несколькими независимыми базами на карте
    {
        ...
        for (int i = 0; i < allResources.Count; i++)
        {
            ...
            if (contains == false)
            {
                ...
                task.Completed += OnTaskCompleted;
                //_taskPool.Add(task);
                // ƒобавить метод сортирующий массив задач по приоритету.
                AddNewTask(task);
            }
        }
    }
    ...
    private Task GetTask()                      // ”празднить ?
    {
    ...
    private IEnumerator ResourceScanning()
    {
        ...
        while (isWork)
        {
            ...
            //if (_freeResources.Count > 0 && _poolOfIdleCollectorBots.Count > 0)             // Ёто заменить на пулл задач где запрос задачь будут делать сами боты?
            DistributeTasks();
        }
